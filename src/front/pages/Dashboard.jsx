...
export const Dashboard = () => {
    ...
    const loadAiSuggestions = async () => {
        try {
            const resp = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/ai/dashboard-suggestions`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (resp.ok) {
                const data = await resp.json();
                ...
                <div className="modal fade" id="linkModal" tabIndex="-1" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 shadow-lg rounded-4">
                            ...
                            <div className="modal-body p-4 text-c   enter">
                    ...
                    {aiSuggestions.length === 0 ? (
                        ...
                        aiSuggestions.map((sug) => (
                            <div key={sug.id} className="border rounded-3 p-3 mb-3" style={{ backgroundColor: "#f9fcf8", borderLeft: "4px solid #198754" }}>
                                ...
                                <p className="small mb-2 text-dark" style={{ lineHeight: "1.5" }}>
                                    {sug.description}
                                </p>
